    ...
    <section class="p-3">
        <div class="information d-flex flex-column gap-5">
            <div class="row px-1">
                <h5>Data Penyakit</h5>
                <div class="wrapper">
                    <form method="post" action="{{ route('admin.update.aturan', $aturan->id) }}" class="mt-6 space-y-6" enctype="multipart/form-data">
                        @csrf
                        @method('put')
                            ...
                            <div class="row justify-content-center">
                                <div class="col-6">
                                    ...
                                    <select name="keputusan"
                                        class="form-select form-select-md mt-2 border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm"
                                        required>
                                        <option disabled>Pilih Keputusan</option>
                                        @foreach ($keputusan as $option)
                                            <option value="{{ $option->kode_keputusan }}"
                                                @if (isset($aturan)) {{ $aturan->keputusan == $option->kode_keputusan ? 'selected' : '' }}@endif>
                                    ...
                                    <select name="penyakit"
                                        ...
                                        @foreach ($penyakit as $option)
                                            <option value="{{ $option->nama_penyakit }}"
                                                @if (isset($aturan)) {{ $aturan->penyakit == $option->nama_penyakit ? 'selected' : '' }}@endif>
                        ...
                        <div>
                            ...
                            <x-text-input id="kriteria_gejala" name="kriteria_gejala" type="text"
                                ...
                                placeholder="Contoh: G001ANDG002ANDG003" value="{{ isset($aturan) ? $aturan->kriteria_gejala : '' }}" />
